...
data_dict = {'Experiment': [], 'Lamarckism_Mean_Ratio': [], 'Darwinism_Mean_Ratio': [], 'P-value': []} #, 'Significance': []
...
for experiment_type in experiments:
    ...
    generation_pairs = [(10, 11), (20, 21)] if experiment_type in ['Flat_2', 'Rugged_2'] else [(5, 6), (10, 11),
                                                                                               (15, 16), (20, 21), (25, 26)]
    lamarckism_ratios = []
    darwinism_ratios = []
    ...
    # # Determine the symbol for p-value significance
    # if p_value <= 1.00e-04:
    #     stars = "****"
    ...
    # data_dict['Significance'].append(stars)
...
ax.set_xlabel('')
# Set a higher y-axis limit
plt.ylim(0, 1.4)  # Adjust the upper limit as needed

# # Annotate plot with significance levels stars
...
#     y, h, col = ratios_df[['Lamarckism_Mean_Ratio', 'Darwinism_Mean_Ratio']].values.max() * 1.03, ratios_df[
#         ['Lamarckism_Mean_Ratio', 'Darwinism_Mean_Ratio']].values.max() * 0.02, 'k'
#
#     plt.plot([x1, x1, x2, x2], [y, y + h, y + h, y], lw=1.5, c=col)
#     plt.text((x1 + x2) * .5, y + h+0.01, f"{row['Significance']}", ha='center', va='bottom', color=col, fontsize=13)
...
plt.tight_layout()
plt.savefig(path+"/Nature_data/plot_images/survivor_transferability_avg.png", bbox_inches='tight')
plt.show()
