...
grouped = df.groupby(['generation', 'experiment', 'individual_id']).agg({'after': 'mean', 'morph_tree_dist': 'mean', 'learning_delta': 'mean'}).reset_index()

filtered_grouped = grouped[grouped['generation'] != 0]

lamarckian_filtered = filtered_grouped[filtered_grouped['experiment'] == f'Lamarckism_{terrain}_{frequency}']
darwinian_filtered = filtered_grouped[filtered_grouped['experiment'] == f'Darwinism_{terrain}_{frequency}']

fig, axs = plt.subplots(1, 2, figsize=(14, 6))

filtered_lamarckian_corr = df[(df['experiment'] == 'Lamarckian+Learning') & (df['generation'] != 0)][['after', 'morph_tree_dist']].corr().iloc[0, 1]
filtered_darwinian_corr = df[(df['experiment'] == 'Darwinian+Learning') & (df['generation'] != 0)][['after', 'morph_tree_dist']].corr().iloc[0, 1]

def plot_2d_density(ax, data, cmap):
    y = data['after']
    x = data['morph_tree_dist']
